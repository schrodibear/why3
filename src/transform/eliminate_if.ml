...
let no_elim_if_l = Ident.create_label "eliminate_if : no"

let no_elim_if t = Slab.mem no_elim_if_l t.t_label

let rec has_if t = match t.t_node with
  | Tif _ when not (no_elim_if t) -> true
  | _ -> TermTF.t_any has_if Util.ffalse t

let rec elim_t contT t =
  if no_elim_if t then contT t
  else
...
and elim_tr contT t = match t.t_node with
  | Tif _ when not (no_elim_if t) ->
      Printer.unsupportedTerm t
        "cannot eliminate a deep 'if-then-else' in trigger term"
